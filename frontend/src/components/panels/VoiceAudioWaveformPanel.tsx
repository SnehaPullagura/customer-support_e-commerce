...
interface VoiceAudioWaveformPanelProps {
  title?: string;
  onRefresh?: () => void;
}

export const VoiceAudioWaveformPanel: React.FC<VoiceAudioWaveformPanelProps> = ({
  title = "Live Omnichannel Voice Audio Waveform Panel",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Visual audio waveform stream with inline speech-to-text keyword tags.</p>
        </div>
        <button
