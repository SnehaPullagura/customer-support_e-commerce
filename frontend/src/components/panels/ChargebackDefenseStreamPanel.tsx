...
interface ChargebackDefenseStreamPanelProps {
  title?: string;
  onRefresh?: () => void;
}

export const ChargebackDefenseStreamPanel: React.FC<ChargebackDefenseStreamPanelProps> = ({
  title = "Stripe & PayPal Chargeback Defense Stream Panel",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Live dispute dossier status tracking evidence submission deadlines.</p>
        </div>
        <button
