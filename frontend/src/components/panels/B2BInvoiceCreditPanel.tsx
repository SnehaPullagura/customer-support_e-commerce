...
interface B2BInvoiceCreditPanelProps {
  title?: string;
  onRefresh?: () => void;
}

export const B2BInvoiceCreditPanel: React.FC<B2BInvoiceCreditPanelProps> = ({
  title = "Wholesale B2B Net-30 Invoice Adjustment Panel",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Credit memo approval queue for corporate accounts.</p>
        </div>
        <button
