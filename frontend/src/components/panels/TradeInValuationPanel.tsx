...
interface TradeInValuationPanelProps {
  title?: string;
  onRefresh?: () => void;
}

export const TradeInValuationPanel: React.FC<TradeInValuationPanelProps> = ({
  title = "Device Trade-In Assessment & Grading Panel",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Real-time grading stream showing cosmetic checks and trade-in credits.</p>
        </div>
        <button
