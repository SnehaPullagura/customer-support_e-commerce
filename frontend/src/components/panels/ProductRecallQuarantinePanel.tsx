...
interface ProductRecallQuarantinePanelProps {
  title?: string;
  onRefresh?: () => void;
}

export const ProductRecallQuarantinePanel: React.FC<ProductRecallQuarantinePanelProps> = ({
  title = "CPSC Product Safety Recall Quarantine Panel",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Hazardous product recall intake queue tracking returned units.</p>
        </div>
        <button
