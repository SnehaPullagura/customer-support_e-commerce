...
interface RealtimeAgentRadarPanelProps {
  title?: string;
  onRefresh?: () => void;
}

export const RealtimeAgentRadarPanel: React.FC<RealtimeAgentRadarPanelProps> = ({
  title = "Realtime Agent State & Queue Occupancy Radar",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Tracks active vs idle agents across technical, billing, and logistics tiers.</p>
        </div>
        <button
