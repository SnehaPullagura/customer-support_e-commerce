...
interface FraudSyndicateAlertPanelProps {
  title?: string;
  onRefresh?: () => void;
}

export const FraudSyndicateAlertPanel: React.FC<FraudSyndicateAlertPanelProps> = ({
  title = "Syndicate Fraud & High-Risk Alert Panel",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Flags suspicious purchasing rings sharing synthetic identities and card hashes.</p>
        </div>
        <button
