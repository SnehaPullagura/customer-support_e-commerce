...
interface PaymentGatewayHealthPanelProps {
  title?: string;
  onRefresh?: () => void;
}

export const PaymentGatewayHealthPanel: React.FC<PaymentGatewayHealthPanelProps> = ({
  title = "Payment Gateway Status & Authorization Rates",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Real-time health monitor tracking Stripe, PayPal, and Adyen capture rates.</p>
        </div>
        <button
