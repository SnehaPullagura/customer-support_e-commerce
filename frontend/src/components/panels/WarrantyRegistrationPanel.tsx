...
interface WarrantyRegistrationPanelProps {
  title?: string;
  onRefresh?: () => void;
}

export const WarrantyRegistrationPanel: React.FC<WarrantyRegistrationPanelProps> = ({
  title = "Serialized Hardware Warranty Registration Panel",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Displays verified device serial numbers and protection plan status.</p>
        </div>
        <button
