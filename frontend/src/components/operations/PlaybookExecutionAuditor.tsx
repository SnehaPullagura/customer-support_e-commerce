...
interface PlaybookExecutionAuditorProps {
  title?: string;
  data?: any;
  onAction?: (payload: any) => void;
}

export const PlaybookExecutionAuditor: React.FC<PlaybookExecutionAuditorProps> = ({
  title = "Resolution Playbook Step Timing Auditor",
  data,
  onAction,
}) => {
    ...
    <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Granular timing breakdown per playbook step highlighting execution bottlenecks.</p>
        </div>
