...
interface DisputeDossierViewerProps {
  title?: string;
  data?: any;
  onAction?: (payload: any) => void;
}

export const DisputeDossierViewer: React.FC<DisputeDossierViewerProps> = ({
  title = "Stripe & PayPal Chargeback Evidence Viewer",
  data,
  onAction,
}) => {
    ...
    <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Split-screen PDF evidence viewer compiling tracking POD and customer checkout logs.</p>
        </div>
