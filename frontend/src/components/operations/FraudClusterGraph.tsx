...
interface FraudClusterGraphProps {
  title?: string;
  data?: any;
  onAction?: (payload: any) => void;
}

export const FraudClusterGraph: React.FC<FraudClusterGraphProps> = ({
  title = "Syndicate Fraud & IP Clustering Graph",
  data,
  onAction,
}) => {
    ...
    <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Network node graph revealing multiple accounts sharing credit card fingerprints.</p>
        </div>
