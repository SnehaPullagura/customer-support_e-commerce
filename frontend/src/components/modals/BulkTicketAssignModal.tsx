...
interface BulkTicketAssignModalProps {
  isOpen?: boolean;
  onClose?: () => void;
...
export const BulkTicketAssignModal: React.FC<BulkTicketAssignModalProps> = ({
  isOpen = true,
  onClose,
  onSubmit,
  title = "Mass Queue Reassignment & Tagging",
}) => {
      ...
      <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl border border-slate-200 space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Reassigns selected batch of tickets to specialized language team.</p>
          </div>
          {onClose && (
