...
interface ChargebackEvidenceModalProps {
  isOpen?: boolean;
  onClose?: () => void;
...
export const ChargebackEvidenceModal: React.FC<ChargebackEvidenceModalProps> = ({
  isOpen = true,
  onClose,
  onSubmit,
  title = "Stripe Dispute Evidence Package Builder",
}) => {
      ...
      <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl border border-slate-200 space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Compiles GPS delivery proof, signature image, and TOS acknowledgment into PDF.</p>
          </div>
          {onClose && (
