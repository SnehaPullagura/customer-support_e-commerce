...
interface DoubleChargeRefundModalProps {
  isOpen?: boolean;
  onClose?: () => void;
...
export const DoubleChargeRefundModal: React.FC<DoubleChargeRefundModalProps> = ({
  isOpen = true,
  onClose,
  onSubmit,
  title = "Duplicate Transaction Void & Reversal",
}) => {
      ...
      <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl border border-slate-200 space-y-4">
        <div className="flex items-center justify-between pb-3 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Reverses duplicate pending credit card authorization hold.</p>
          </div>
          {onClose && (
