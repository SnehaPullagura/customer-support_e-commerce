...
interface LiveAgentQueueGaugeProps {
  title?: string;
  onRefresh?: () => void;
}

export const LiveAgentQueueGauge: React.FC<LiveAgentQueueGaugeProps> = ({
  title = "Live Agent Queue Occupancy & Real-Time Capacity Gauge",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Dynamic gauge monitoring agent capacity and active chat saturation across queues.</p>
        </div>
        <button
