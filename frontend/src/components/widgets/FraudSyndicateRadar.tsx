...
interface FraudSyndicateRadarProps {
  title?: string;
  onRefresh?: () => void;
}

export const FraudSyndicateRadar: React.FC<FraudSyndicateRadarProps> = ({
  title = "Syndicate Fraud Ring Detection & Card Velocity Radar",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Radar chart highlighting velocity anomalies and linked customer accounts.</p>
        </div>
        <button
