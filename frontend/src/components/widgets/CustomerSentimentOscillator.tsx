...
interface CustomerSentimentOscillatorProps {
  title?: string;
  onRefresh?: () => void;
}

export const CustomerSentimentOscillator: React.FC<CustomerSentimentOscillatorProps> = ({
  title = "Live Customer Sentiment & Frustration Waveform",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Continuous waveform chart visualizing emotional valence and agent empathy score.</p>
        </div>
        <button
