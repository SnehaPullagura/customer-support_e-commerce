...
interface InventoryBackorderAlertPillProps {
  title?: string;
  onRefresh?: () => void;
}

export const InventoryBackorderAlertPill: React.FC<InventoryBackorderAlertPillProps> = ({
  title = "Warehouse Inventory Stockout & Substitution Pill",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Warning badge identifying out-of-stock items needing complimentary SKU upgrades.</p>
        </div>
        <button
