...
interface ProductRecallSafetyBannerProps {
  title?: string;
  onRefresh?: () => void;
}

export const ProductRecallSafetyBanner: React.FC<ProductRecallSafetyBannerProps> = ({
  title = "CPSC Product Safety Recall Mandatory Quarantine Banner",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Emergency safety notice banner displaying certified hazardous return packing instructions.</p>
        </div>
        <button
