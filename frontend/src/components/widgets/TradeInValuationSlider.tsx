...
interface TradeInValuationSliderProps {
  title?: string;
  onRefresh?: () => void;
}

export const TradeInValuationSlider: React.FC<TradeInValuationSliderProps> = ({
  title = "Device Trade-In Cosmetic Grading & Value Slider",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Adjustable slider calculating instant trade-in credit based on screen and battery grade.</p>
        </div>
        <button
