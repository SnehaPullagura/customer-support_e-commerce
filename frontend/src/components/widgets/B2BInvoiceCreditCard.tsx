...
interface B2BInvoiceCreditCardProps {
  title?: string;
  onRefresh?: () => void;
}

export const B2BInvoiceCreditCard: React.FC<B2BInvoiceCreditCardProps> = ({
  title = "Wholesale B2B Net-30 Invoice Discrepancy Card",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Interactive invoice line-item editor for authorizing corporate credit adjustments.</p>
        </div>
        <button
