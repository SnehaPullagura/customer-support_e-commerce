...
interface PlaybookExecutionStopwatchProps {
  title?: string;
  onRefresh?: () => void;
}

export const PlaybookExecutionStopwatch: React.FC<PlaybookExecutionStopwatchProps> = ({
  title = "Resolution Playbook Step Completion Stopwatch",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Granular stopwatch measuring latency per execution step in active resolution playbooks.</p>
        </div>
        <button
