...
interface ChargebackDefenseDossierCardProps {
  title?: string;
  onRefresh?: () => void;
}

export const ChargebackDefenseDossierCard: React.FC<ChargebackDefenseDossierCardProps> = ({
  title = "Stripe & PayPal Dispute Defense Dossier Card",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Structured summary card compiling proof of delivery photos and IP logs for dispute review.</p>
        </div>
        <button
