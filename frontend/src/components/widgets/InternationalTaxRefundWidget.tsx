...
interface InternationalTaxRefundWidgetProps {
  title?: string;
  onRefresh?: () => void;
}

export const InternationalTaxRefundWidget: React.FC<InternationalTaxRefundWidgetProps> = ({
  title = "Cross-Border Duty & Tourist VAT Rebate Widget",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Calculates export duty drawbacks and displays customs stamp validation status.</p>
        </div>
        <button
