...
interface CarrierTransitSpeedometerProps {
  title?: string;
  onRefresh?: () => void;
}

export const CarrierTransitSpeedometer: React.FC<CarrierTransitSpeedometerProps> = ({
  title = "Carrier Transit Speed & On-Time Delivery Speedometer",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Visual speedometer comparing actual delivery times against guaranteed carrier SLAs.</p>
        </div>
        <button
