...
interface OmnichannelInflowVelocityProps {
  title?: string;
  onRefresh?: () => void;
}

export const OmnichannelInflowVelocity: React.FC<OmnichannelInflowVelocityProps> = ({
  title = "Multi-Channel Ticket Inflow & Routing Velocity Gauge",
  onRefresh,
}) => {
    ...
    <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-xs space-y-4">
      <div className="flex items-center justify-between pb-3 border-b border-slate-100">
        <div>
          ...
          <p className="text-[11px] text-slate-500 mt-0.5">Interactive gauge showing live ticket ingestion volume across WhatsApp, SMS, and Email.</p>
        </div>
        <button
