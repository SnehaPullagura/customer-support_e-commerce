"""
Intercom Messenger Real-Time Event Sync Gateway (INTERCOM)
...
class IntercomMessengerMessagePayload(BaseModel):
    message_id: str
    channel_code: str = "INTERCOM"
    sender_id: str
    recipient_id: str
    content_text: str
...
class IntercomMessenger:
    """Omnichannel transport adapter for Intercom Messenger Real-Time Event Sync Gateway."""
    CHANNEL_CODE = "INTERCOM"
    CHANNEL_TITLE = "Intercom Messenger Real-Time Event Sync Gateway"
    ...
    async def parse_inbound_webhook(self, raw_json: Dict[str, Any]) -> IntercomMessengerMessagePayload:
        return IntercomMessengerMessagePayload(
