"""
Inbound RFC-822 MIME Email & SPF/DKIM Verifier (EMAIL_MIME)
...
class EmailMimeParserMessagePayload(BaseModel):
    message_id: str
    channel_code: str = "EMAIL_MIME"
    sender_id: str
    recipient_id: str
    content_text: str
...
class EmailMimeParser:
    """Omnichannel transport adapter for Inbound RFC-822 MIME Email & SPF/DKIM Verifier."""
    CHANNEL_CODE = "EMAIL_MIME"
    CHANNEL_TITLE = "Inbound RFC-822 MIME Email & SPF/DKIM Verifier"
    ...
    async def parse_inbound_webhook(self, raw_json: Dict[str, Any]) -> EmailMimeParserMessagePayload:
        return EmailMimeParserMessagePayload(
