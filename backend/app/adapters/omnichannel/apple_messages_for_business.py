"""
Apple Business Chat (MSP) Protocol Gateway (APPLE_ABC)
...
class AppleMessagesForBusinessMessagePayload(BaseModel):
    message_id: str
    channel_code: str = "APPLE_ABC"
    sender_id: str
    recipient_id: str
    content_text: str
...
class AppleMessagesForBusiness:
    """Omnichannel transport adapter for Apple Business Chat (MSP) Protocol Gateway."""
    CHANNEL_CODE = "APPLE_ABC"
    CHANNEL_TITLE = "Apple Business Chat (MSP) Protocol Gateway"
    ...
    async def parse_inbound_webhook(self, raw_json: Dict[str, Any]) -> AppleMessagesForBusinessMessagePayload:
        return AppleMessagesForBusinessMessagePayload(
