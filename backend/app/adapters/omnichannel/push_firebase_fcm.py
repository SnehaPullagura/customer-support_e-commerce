"""
Firebase Cloud Messaging (FCM) Push Gateway (FCM)
...
class PushFirebaseFcmMessagePayload(BaseModel):
    message_id: str
    channel_code: str = "FCM"
    sender_id: str
    recipient_id: str
    content_text: str
...
class PushFirebaseFcm:
    """Omnichannel transport adapter for Firebase Cloud Messaging (FCM) Push Gateway."""
    CHANNEL_CODE = "FCM"
    CHANNEL_TITLE = "Firebase Cloud Messaging (FCM) Push Gateway"
    ...
    async def parse_inbound_webhook(self, raw_json: Dict[str, Any]) -> PushFirebaseFcmMessagePayload:
        return PushFirebaseFcmMessagePayload(
