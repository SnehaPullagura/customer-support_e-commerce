"""
LiveChat Inc v3.5 WebSocket Webhook Gateway (LIVECHAT)
...
class LivechatIncGatewayMessagePayload(BaseModel):
    message_id: str
    channel_code: str = "LIVECHAT"
    sender_id: str
    recipient_id: str
    content_text: str
...
class LivechatIncGateway:
    """Omnichannel transport adapter for LiveChat Inc v3.5 WebSocket Webhook Gateway."""
    CHANNEL_CODE = "LIVECHAT"
    CHANNEL_TITLE = "LiveChat Inc v3.5 WebSocket Webhook Gateway"
    ...
    async def parse_inbound_webhook(self, raw_json: Dict[str, Any]) -> LivechatIncGatewayMessagePayload:
        return LivechatIncGatewayMessagePayload(
