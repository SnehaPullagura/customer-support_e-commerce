"""
LINE Official Account Messaging API Gateway (LINE)
...
class LineMessagingMessagePayload(BaseModel):
    message_id: str
    channel_code: str = "LINE"
    sender_id: str
    recipient_id: str
    content_text: str
...
class LineMessaging:
    """Omnichannel transport adapter for LINE Official Account Messaging API Gateway."""
    CHANNEL_CODE = "LINE"
    CHANNEL_TITLE = "LINE Official Account Messaging API Gateway"
    ...
    async def parse_inbound_webhook(self, raw_json: Dict[str, Any]) -> LineMessagingMessagePayload:
        return LineMessagingMessagePayload(
