"""
Genesys Cloud CX Inbound Voice & Routing Gateway (GENESYS)
...
class VoiceGenesysCloudMessagePayload(BaseModel):
    message_id: str
    channel_code: str = "GENESYS"
    sender_id: str
    recipient_id: str
    content_text: str
...
class VoiceGenesysCloud:
    """Omnichannel transport adapter for Genesys Cloud CX Inbound Voice & Routing Gateway."""
    CHANNEL_CODE = "GENESYS"
    CHANNEL_TITLE = "Genesys Cloud CX Inbound Voice & Routing Gateway"
    ...
    async def parse_inbound_webhook(self, raw_json: Dict[str, Any]) -> VoiceGenesysCloudMessagePayload:
        return VoiceGenesysCloudMessagePayload(
