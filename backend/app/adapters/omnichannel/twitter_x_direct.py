"""
X / Twitter Direct Messages Support Gateway (TWITTER_DM)
...
class TwitterXDirectMessagePayload(BaseModel):
    message_id: str
    channel_code: str = "TWITTER_DM"
    sender_id: str
    recipient_id: str
    content_text: str
...
class TwitterXDirect:
    """Omnichannel transport adapter for X / Twitter Direct Messages Support Gateway."""
    CHANNEL_CODE = "TWITTER_DM"
    CHANNEL_TITLE = "X / Twitter Direct Messages Support Gateway"
    ...
    async def parse_inbound_webhook(self, raw_json: Dict[str, Any]) -> TwitterXDirectMessagePayload:
        return TwitterXDirectMessagePayload(
