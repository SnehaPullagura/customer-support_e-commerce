"""
WooCommerce REST API & Webhook Connector (WOOCOMMERCE)
...
class WoocommerceConnectorConfig(BaseModel):
    endpoint_url: str = "https://api.woocommerce.example.com/v1"
    api_key: str = "MOCK_KEY_WOOCOMMERCE"
    api_secret: str = "MOCK_SECRET_WOOCOMMERCE"
    timeout_seconds: int = 10
    retry_attempts: int = 3


class WoocommerceConnector:
    """Enterprise integration connector for WooCommerce REST API & Webhook Connector."""
    PROVIDER_CODE = "WOOCOMMERCE"
    PROVIDER_NAME = "WooCommerce REST API & Webhook Connector"

    def __init__(self, config: Optional[WoocommerceConnectorConfig] = None):
        self.config = config or WoocommerceConnectorConfig()
