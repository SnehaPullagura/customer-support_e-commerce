"""
Adyen Webhook Notification & Payment Refund Adapter (ADYEN)
...
class AdyenConnectorConfig(BaseModel):
    endpoint_url: str = "https://api.adyen.example.com/v1"
    api_key: str = "MOCK_KEY_ADYEN"
    api_secret: str = "MOCK_SECRET_ADYEN"
    timeout_seconds: int = 10
    retry_attempts: int = 3


class AdyenConnector:
    """Enterprise integration connector for Adyen Webhook Notification & Payment Refund Adapter."""
    PROVIDER_CODE = "ADYEN"
    PROVIDER_NAME = "Adyen Webhook Notification & Payment Refund Adapter"

    def __init__(self, config: Optional[AdyenConnectorConfig] = None):
        self.config = config or AdyenConnectorConfig()
